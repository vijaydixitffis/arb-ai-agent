      ...
      <div className="relative w-full max-w-sm bg-white h-full shadow-2xl flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {loading ? (
            ...
            <div key={h.id}
              className={`border rounded-lg px-3 py-2.5 ${h.is_active ? 'border-teal-300 bg-teal-50' : 'border-slate-100'}`}>
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-xs font-semibold text-slate-700">v{h.version}</span>
                  {h.is_active && <span className="ml-2 text-xs text-teal-600 font-medium">• active</span>}
                </div>
                {!h.is_active && (
                  <button
                    onClick={() => handleRevert(h.version)}
                    disabled={reverting === h.version}
                    className="text-xs px-2 py-1 border border-slate-200 rounded text-slate-600 hover:bg-teal-50 hover:border-teal-300 hover:text-teal-700 disabled:opacity-40"
                  >
          ...
          <div className="border-t border-slate-100 p-5 space-y-3">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                disabled={!dirty}
                className="flex-1 h-9 px-3 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 disabled:bg-slate-50 disabled:text-slate-400"
              />
              <button
                onClick={handleSave}
                disabled={!dirty || saving}
                className="flex items-center gap-1.5 px-4 h-9 bg-teal-600 text-white rounded-lg text-sm font-semibold hover:bg-teal-700 disabled:opacity-40 disabled:cursor-not-allowed"
              >
    ...
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 h-9 border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-slate-50 hover:border-teal-300 hover:text-teal-700 disabled:opacity-40 disabled:cursor-not-allowed"
